...
public class ObjectMarker {
    ...
    public class DfsThread implements Runnable {
        int[] data = new int[VideoPreloadSizeExperiment.DEFAULT];
        IntStack rootsStack;
        int size;
    ...
    public int markSingleThreaded(ExcludedReferencesDescriptor[] excludedReferencesDescriptorArr, ISnapshot iSnapshot) throws SnapshotException, OperationCanceledException {
        ...
        while (i6 > 0) {
            ...
            int i14 = i10;
            int[] iArr8 = iArr7;
            for (int i15 : this.outbound.get(i11)) {
                if (!this.bits[i15]) {
                    iArr2 = iArr8;
                    i2 = i14;
                    if (!refersOnlyThroughExcluded(i11, i15, excludedReferencesDescriptorArr, bitField, iSnapshot)) {
                        if (i2 == iArr2.length) {
                            z = true;
                            iArr8 = new int[(iArr2.length << 1)];

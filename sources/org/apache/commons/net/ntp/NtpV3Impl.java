...
public class NtpV3Impl implements NtpV3Packet {
    ...
    /* renamed from: dp */
    private volatile DatagramPacket f133245dp;

    /* renamed from: ui */
    protected static final int m114284ui(byte b) {
        return b & 255;
    }

    /* renamed from: ul */
    protected static final long m114285ul(byte b) {
        return (long) (b & 255);
    }
    ...
    public int getMode() {
        return (m114284ui(this.buf[0]) >> 0) & 7;
    }
    ...
    public int getStratum() {
        return m114284ui(this.buf[1]);
    }

    public int getVersion() {
        return (m114284ui(this.buf[0]) >> 3) & 7;
    }
    ...
    public synchronized DatagramPacket getDatagramPacket() {
        if (this.f133245dp == null) {
            this.f133245dp = new DatagramPacket(this.buf, this.buf.length);
            this.f133245dp.setPort(123);
        }
        return this.f133245dp;
    }
    ...
    private String idAsIPAddress() {
        StringBuilder sb = new StringBuilder();
        sb.append(m114284ui(this.buf[12]));
        sb.append(ClassUtils.PACKAGE_SEPARATOR);
        sb.append(m114284ui(this.buf[13]));
        sb.append(ClassUtils.PACKAGE_SEPARATOR);
        sb.append(m114284ui(this.buf[14]));
        sb.append(ClassUtils.PACKAGE_SEPARATOR);
        sb.append(m114284ui(this.buf[15]));
        return sb.toString();
    }
    ...
    private int getInt(int i) {
        return m114284ui(this.buf[i + 3]) | (m114284ui(this.buf[i]) << 24) | (m114284ui(this.buf[i + 1]) << 16) | (m114284ui(this.buf[i + 2]) << 8);
    }

    private long getLong(int i) {
        return (m114285ul(this.buf[i]) << 56) | (m114285ul(this.buf[i + 1]) << 48) | (m114285ul(this.buf[i + 2]) << 40) | (m114285ul(this.buf[i + 3]) << 32) | (m114285ul(this.buf[i + 4]) << 24) | (m114285ul(this.buf[i + 5]) << 16) | (m114285ul(this.buf[i + 6]) << 8) | m114285ul(this.buf[i + 7]);
    }

...
public class LiveStreamAudioCapture extends AudioCapturer implements AudioCaptureObserver {
    ...
    public void onReleaseCapture() {
        if (this.mAudioCapturer != null) {
            ...
            if (!(this.mOldAudioManagerMode == -2 || this.mOldAudioManagerMode == AudioDeviceModule.getMode())) {
                StringBuilder sb = new StringBuilder("AudioManager restore mode:");
                sb.append(this.mOldAudioManagerMode);
                AVLog.ioi("LiveStreamAudioCapture", sb.toString());
                AudioDeviceModule.setMode(this.mOldAudioManagerMode);
            }
            this.mOldAudioManagerMode = -2;
        }
    ...
    private void onCreateCapture(int i) {
        ...
        if (getMode() == 7) {
            ...
            if (mode != 3) {
                this.mOldAudioManagerMode = mode;
                StringBuilder sb = new StringBuilder("AudioManager setmode(COMMUNICATION), old AudioManager mode:");
                sb.append(this.mOldAudioManagerMode);
                AVLog.ioi("LiveStreamAudioCapture", sb.toString());

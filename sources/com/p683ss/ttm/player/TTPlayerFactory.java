...
import com.p683ss.ttm.utils.PhoneInfo;
import com.p683ss.ttm.utils.app.ServiceUtil;
...
class TTPlayerFactory implements Callback {
    ...
    public C0785g<TTPlayerIPCRef> mPlayers = new C0785g<>();
    ...
    public void onServiceDisconnected() {
        ...
        for (int i = 0; i < b; i++) {
            TTPlayerIPCRef tTPlayerIPCRef = (TTPlayerIPCRef) this.mPlayers.mo2494c(i);
    ...
    private TTPlayerIPCRef getPlayer(long j) {
        ...
        if (j > 0) {
            try {
                tTPlayerIPCRef = (TTPlayerIPCRef) this.mPlayers.mo2487a(j);
            } catch (Throwable unused) {
                return null;
            }
        } else {
            tTPlayerIPCRef = (TTPlayerIPCRef) this.mPlayers.mo2494c(0);
        }
    ...
    public TTPlayerIPCRef createPlayer(Context context, HashMap<Integer, Integer> hashMap) throws Exception {
        if (this.mConnection == null && !startService(hashMap)) {
            ...
            this.mPlayers.mo2490b();
            this.mPlayers.mo2493b(tTPlayerIPCRef.getId(), tTPlayerIPCRef);
            this.mHandler.removeMessages(10000);
            return tTPlayerIPCRef;
        }
    }

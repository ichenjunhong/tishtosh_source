        ...
        if ("android.intent.action.HEADSET_PLUG".equals(intent.getAction()) && intent.hasExtra("state")) {
            ...
            if (intExtra == 1) {
                if (intent.getIntExtra("microphone", -1) == 1) {
                    LogUtil.m110867i("WiredHeadsetBroadcastReceiver", "Headset w/ mic connected");
                    this.mDeviceManager.onDeviceOnline();
                    this.mDeviceManager.reportHeadsetType(0);
                } else {
                    this.mDeviceManager.onDeviceOnline();
                    this.mDeviceManager.reportHeadsetType(1);
                    LogUtil.m110867i("WiredHeadsetBroadcastReceiver", "Headset w/o mic connected");
                }
            } else if (intExtra == 0) {
                LogUtil.m110867i("WiredHeadsetBroadcastReceiver", "Headset disconnected");
                this.mDeviceManager.onDeviceOffline();
            } else {
                StringBuilder sb = new StringBuilder("Headset unknown event detected, state=");
                sb.append(intExtra);
                LogUtil.m110867i("WiredHeadsetBroadcastReceiver", sb.toString());
            }
        }
    }
}

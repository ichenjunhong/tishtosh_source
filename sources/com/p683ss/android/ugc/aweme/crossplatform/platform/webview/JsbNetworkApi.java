package com.p683ss.android.ugc.aweme.crossplatform.platform.webview;

import com.bytedance.retrofit2.C12690b;
import com.bytedance.retrofit2.p830a.C12685b;
import com.bytedance.retrofit2.p831b.C12691a;
import com.bytedance.retrofit2.p831b.C12694aa;
import com.bytedance.retrofit2.p831b.C12699af;
import com.bytedance.retrofit2.p831b.C12702d;
import com.bytedance.retrofit2.p831b.C12704f;
import com.bytedance.retrofit2.p831b.C12705g;
import com.bytedance.retrofit2.p831b.C12710l;
import com.bytedance.retrofit2.p831b.C12713o;
import com.bytedance.retrofit2.p831b.C12718t;
import java.util.List;
import java.util.Map;

/* renamed from: com.ss.android.ugc.aweme.crossplatform.platform.webview.JsbNetworkApi */
public interface JsbNetworkApi {
    @C12718t
    @C12705g
    C12690b<String> doPost(@C12713o int i, @C12699af String str, @C12694aa Map<String, String> map, @C12704f(mo23875a = true) Map<String, String> map2, @C12710l List<C12685b> list, @C12702d Object obj, @C12691a boolean z);
}

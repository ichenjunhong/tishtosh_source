...
public class CircleProgress extends View {
    ...
    private C11136a[] mArcPoint;
    private Point mCenter = new Point();
    private long mDuration = 3600;
    private TimeInterpolator mInterpolator = new C11137b();
    ...
    static class C11136a {
        ...
        C11136a(float f, float f2, int i) {
            this.f30026a = f;
            this.f30027b = f2;
            this.f30028c = i;
        }
    ...
    static class C11137b implements TimeInterpolator {
        C11137b() {
        }
    ...
    private void calPoints(float f) {
        ...
        for (int i2 = 0; i2 < 8; i2++) {
            ...
            this.mArcPoint[i2] = new C11136a((-((float) Math.sin(d2))) * f2, f2 * (-((float) Math.cos(d2))), this.COLORS[i2 % 3]);
        }
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < 8; i++) {
            this.mPaint.setColor(this.mArcPoint[i].f30028c);
            float itemFactor = getItemFactor(i, factor);
            canvas.drawCircle(this.mArcPoint[i].f30026a - ((this.mArcPoint[i].f30026a * 2.0f) * itemFactor), this.mArcPoint[i].f30027b - ((this.mArcPoint[i].f30027b * 2.0f) * itemFactor), (float) this.mPointRadius, this.mPaint);
        }
        canvas.restore();
        if (this.mStartAnim) {
            postInvalidate();
    ...
    public void onMeasure(int i, int i2) {
        int dimensionPixelSize = getResources().getDimensionPixelSize(R.dimen.f_);
    ...
    private void init(AttributeSet attributeSet, int i) {
        this.mArcPoint = new C11136a[8];
        this.mPaint = new Paint();
        this.mPaint.setAntiAlias(true);
        this.mPaint.setStyle(Style.FILL);
        TypedArray obtainStyledAttributes = getContext().obtainStyledAttributes(attributeSet, new int[]{R.attr.fn, R.attr.fo, R.attr.fp}, i, 0);

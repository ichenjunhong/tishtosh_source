...
public class CronetHttpURLConnection extends HttpURLConnection {
        ...
        private void setRequestEnd() {
            ...
            try {
                if (CronetHttpURLConnection.this.mResponseInfo != null) {
                    ...
                    if (CronetHttpURLConnection.this.mCookieHandler != null) {
                        if (TextUtils.isEmpty(CronetHttpURLConnection.this.mFinalUrl)) {
                            ...
                            CronetHttpURLConnection.this.mCookieHandler.put(CronetHttpURLConnection.this.uri(new URL(CronetHttpURLConnection.this.mFinalUrl)), allHeaders);
                        }
                    }
                }
            } catch (Exception e) {
                if (Logger.debug()) {
                    ...
                    Logger.m111346d(str, sb.toString());
                }
            }
            try {
                if (CronetHttpURLConnection.this.mException != null) {
                    ...
                    if (!TextUtils.isEmpty(str2)) {
                        ...
                        sb3.append(message);
                        Reflect.m111362on((Object) CronetHttpURLConnection.this.mException).set("detailMessage", sb3.toString());
                    }
                }
                ...
                if (CronetHttpURLConnection.this.instanceFollowRedirects) {
                    if (CronetHttpURLConnection.this.mRequest != null) {
                        if (!equals) {
                            ...
                            Map GetCookieMap = CronetHttpURLConnection.this.GetCookieMap(CronetHttpURLConnection.this.uri(new URL(str)));
                            if (GetCookieMap != null && !GetCookieMap.isEmpty()) {
                                for (Entry entry : GetCookieMap.entrySet()) {
        ...
        if (this.mOutputStream == null && this.doOutput) {
            if (this.connected) {
                ...
                if (streamingModeContentLength != -1) {
                    ...
                    Log.m111326d(TAG, "Outputstream is being buffered in memory.");
    ...
    private void startRequest() throws IOException {
        if (!this.connected) {
            ...
            Map GetCookieMap = GetCookieMap(uri(getURL()));
            if (GetCookieMap != null && !GetCookieMap.isEmpty()) {
                for (Entry entry : GetCookieMap.entrySet()) {
    ...
    public Map<String, String> GetCookieMap(URI uri) {
        ...
        try {
            ...
            if (this.mCookieHandler != null) {
                map = this.mCookieHandler.get(uri, hashMap2);
            }
        } catch (Exception e) {
            if (Logger.debug()) {
                String str3 = TAG;
                ...
                sb.append(" e = ");
                sb.append(e.getMessage());
                Logger.m111346d(str3, sb.toString());
            }
        }
        if (map != null && map.size() > 0) {
            for (Entry entry : map.entrySet()) {
                ...
                StringBuilder sb2 = new StringBuilder();
                if (("X-SS-Cookie".equalsIgnoreCase(str4) || "Cookie".equalsIgnoreCase(str4) || "Cookie2".equalsIgnoreCase(str4)) && !((List) entry.getValue()).isEmpty()) {
                    int i = 0;

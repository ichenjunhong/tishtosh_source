...
public class HttpNegotiateAuthenticator {
        ...
        public void run(AccountManagerFuture<Account[]> accountManagerFuture) {
            try {
                ...
                if (accountArr.length == 0) {
                    Log.m111344w("net_auth", "ERR_MISSING_AUTH_CREDENTIALS: No account provided for the kerberos authentication. Please verify the configuration policies and that the CONTACTS runtime permission is granted. ", new Object[0]);
                    HttpNegotiateAuthenticator.this.nativeSetResult(this.mRequestData.nativeResultObject, -341, null);
                } else if (accountArr.length > 1) {
                    Log.m111344w("net_auth", "ERR_MISSING_AUTH_CREDENTIALS: Found %d accounts eligible for the kerberos authentication. Please fix the configuration by providing a single account.", Integer.valueOf(accountArr.length));
                    HttpNegotiateAuthenticator.this.nativeSetResult(this.mRequestData.nativeResultObject, -341, null);
                    ...
                    Log.m111334e("net_auth", "ERR_MISCONFIGURED_AUTH_ENVIRONMENT: USE_CREDENTIALS permission not granted. Aborting authentication.", new Object[0]);
                ...
                Log.m111344w("net_auth", "ERR_UNEXPECTED: Error while attempting to retrieve accounts.", e);
    ...
    class GetTokenCallback implements AccountManagerCallback<Bundle> {
        ...
        public void run(AccountManagerFuture<Bundle> accountManagerFuture) {
            try {
                ...
                Log.m111344w("net_auth", "ERR_UNEXPECTED: Error while attempting to obtain a token.", e);
                HttpNegotiateAuthenticator.this.nativeSetResult(this.mRequestData.nativeResultObject, -9, null);
            }
        }
    ...
    private void requestTokenWithoutActivity(Context context, RequestData requestData, String[] strArr) {
        if (lacksPermission(context, "android.permission.GET_ACCOUNTS", true)) {
            Log.m111334e("net_auth", "ERR_MISCONFIGURED_AUTH_ENVIRONMENT: GET_ACCOUNTS permission not granted. Aborting authentication.", new Object[0]);
    ...
    private void requestTokenWithActivity(Context context, Activity activity, RequestData requestData, String[] strArr) {
        ...
        if (lacksPermission(context, str, z)) {
            Log.m111334e("net_auth", "ERR_MISCONFIGURED_AUTH_ENVIRONMENT: %s permission not granted. Aborting authentication", str);
            nativeSetResult(requestData.nativeResultObject, -343, null);
            return;
        }

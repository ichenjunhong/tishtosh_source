...
public class ChildProcessLauncher {
    ...
    private void setupConnection() {
        C519712 r0 = new ConnectionCallback() {
    ...
    public void stop() {
        Log.m111327d("ChildProcLauncher", "stopping child connection: pid=%d", Integer.valueOf(this.mConnection.getPid()));
        this.mConnection.stop();
    }

    public void onServiceConnected(ChildProcessConnection childProcessConnection) {
        Log.m111327d("ChildProcLauncher", "on connect callback, pid=%d", Integer.valueOf(this.mConnection.getPid()));
        this.mDelegate.onConnectionEstablished(this.mConnection);
        try {
            for (FileDescriptorInfo fileDescriptorInfo : this.mFilesToBeMapped) {
                fileDescriptorInfo.f129615fd.close();
            }
        } catch (IOException e) {
            Log.m111344w("ChildProcLauncher", "Failed to close FD.", e);
        }
    }
    ...
    public boolean start(final boolean z, final boolean z2) {
        try {
            ...
            C519691 r0 = new ServiceCallback() {
                static final /* synthetic */ boolean $assertionsDisabled = false;
                ...
                public void onChildStartFailed(ChildProcessConnection childProcessConnection) {
                    Log.m111334e("ChildProcLauncher", "ChildProcessConnection.start failed, trying again", new Object[0]);
    ...
    public boolean lambda$allocateAndSetupConnection$0$ChildProcessLauncher(ServiceCallback serviceCallback, boolean z, boolean z2) {
        ...
        if (this.mConnection != null) {
            ...
            Log.m111326d("ChildProcLauncher", "Failed to allocate a child connection (no queuing).");
            return false;
        } else {
